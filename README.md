...
![SmartPark 3D Parking Simulator](assets/images/smartpark-overview.png)

SmartPark provides an interactive multi-level parking environment where the user can drive a vehicle, visualize parking-space availability, select a destination, and receive route guidance inside the parking facility.
...
### 3D Environment and Vehicle

![Vehicle inside SmartPark environment](assets/images/vehicle-parking-environment.png)

The vehicle interacts with the multi-level parking structure, ramps, parking areas, structural elements, and navigation system in real time.
...
![Parking Space Navigation](assets/images/parking-route.png)

The calculated route is visualized directly inside the 3D environment, guiding the vehicle toward the selected parking space.

### Route to the Exit

![Exit Route](assets/images/exit-route.png)

The navigation system can also calculate a route from the current vehicle position toward the parking facility exit.
